...
class TestCountDataTypes(unittest.TestCase):
    def test_count_data_types(self):
        ...
        result = count_data_types(data)
        self.assertEqual(result, {'int': 10, 'float': 0, 'str': 0, 'bool': 0, 'list': 0, 'tuple': 0, 'dict': 0, 'set': 0})

        data = [1.0, 2.5, 3.14, 4.2, 5.0]
        result = count_data_types(data)
        self.assertEqual(result, {'int': 0, 'float': 5, 'str': 0, 'bool': 0, 'list': 0, 'tuple': 0, 'dict': 0, 'set': 0})

        data = ['hello', 'world', 'python', 'unittest']
        result = count_data_types(data)
        self.assertEqual(result, {'int': 0, 'float': 0, 'str': 4, 'bool': 0, 'list': 0, 'tuple': 0, 'dict': 0, 'set': 0})
